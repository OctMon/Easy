...
public struct EasyCheck {
    ...
    static func requestPgyerBeta(api_key: String, shortcutUrl: String, headerImage: UIImage?) {
        ...
        session.post(path: "apiv2/app/getByShortcut", isURLEncoding: true, parameters: ["_api_key": api_key, "buildShortcutUrl": shortcutUrl]) { (dataResponse) in
            ...
            EasyApp.showAlert(image: headerImage, title: "发现新版本beta".getAttributedString(font: .size21, foregroundColor: .hex333333).append(title: "  v\(buildVersionNo)", font: .size12, foregroundColor: .hex999999), message: dataResponse.dataParameters["buildUpdateDescription"].toStringValue.getAttributedString(font: .size14, foregroundColor: .hex999999).append(lineSpacing: 8), buttonTitles: buttonTitles, buttonBackgroundImages: buttonBackgroundImages, tap: { offset in
                isShow = false
                if offset == 1 {
