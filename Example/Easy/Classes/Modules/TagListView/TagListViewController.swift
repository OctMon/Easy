...
class TagListViewController: easy.ViewController, easy.TableListProtocol {
    ...
    override func configure() {
        ...
        tableListView.register(String.self, cellClass:UITableViewCell.self, configureCell: { (listView, cell, indexPath, any) in
            ...
            let tagListView = easy.TagListView(frame: CGRect(x: 0, y: 0, width: .screenWidth, height: .screenWidth)).then {
                ...
                $0.borderWidth = CGFloat.random(in: 1..<5)
                $0.font = UIFont.systemFont(ofSize: CGFloat.random(in: 20..<80))
            }
            tagListView.setTags(tags, tap: { (offset) in
